...
function initOptions(options) {
    ...
    if (options.width == null) {
        options.width =
            ((_a = document.getElementById(options.elementId)) === null || _a === void 0 ? void 0 : _a.offsetWidth) || 500;
    }
    if (options.height == null) {
        options.height =
            ((_b = document.getElementById(options.elementId)) === null || _b === void 0 ? void 0 : _b.offsetHeight) || 500;
    }
    // set default margin
...
const labelFontWidth$1 = 6.021666;
class WaterfallPlot {
    constructor(options) {
    ...
    init() {
        // Init svg
        this.svg = select(this.options.elementId).append('svg');
        ...
        let minValue = Math.min(currValue, currValue + data[0][0]);
        let maxValue = Math.max(currValue, currValue + data[0][0]);
        (this.options.reverse ? reversedData : data).forEach((datapoint) => {
            currValue += datapoint[0];
        ...
        if (this.options.orientation === 'h') {
            ...
            const yAxisWidth = Math.max(...data.map((e) => e[1].length)) * labelFontWidth$1
            ...
            if (!this.options.xAxis.hidden) {
                ...
                if (this.options.reverse) {
                    ...
                    for (let i = this.options.data.x.length - 1; i >= 0; --i) {
                        const xVal = this.options.data.x[i];
                        const yVal = this.options.data.y[i];
                        let bar;
...
class ShapPlot {
    ...
    init() {
        ...
        data.sort((a, b) => Math.abs(a[0]) > Math.abs(b[0]) ? -1 : 1);
        ...
        const guideLabel = svg.append('g')
            .append('text')
            .attr('x', Math.min(this.options.width - this.options.margin.r, x(lastValue) + font10pxWidth * (5 + lastValue.toFixed(3).length) / 2))
...
exports.ShapPlot = ShapPlot;
//# sourceMappingURL=plotlab.cjs.js.map
